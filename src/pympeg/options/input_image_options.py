...
from pympeg.utils.validation import validate_choices, time_to_string, validate_positive_number
...
class InputImageOptions(Options):
    ...
    # ========== PROPERTY: loop ==========
    @property
    def loop(self) -> int | None: return self._loop

    @loop.setter
    @validate_choices(LOOP_VALUES)
    def loop(self, value: int) -> None: self._loop = value

    @loop.deleter
    def loop(self) -> None: self._loop = None
